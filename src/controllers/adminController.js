...
const activateStudent = async (req, res) => {
  ...
  try {
    ...
    await prisma.zedCredit.upsert({
      where: { studentId },
      update: {},
      create: { studentId, balance: 0, fundBalance: 0, escrowLocked: true }
    });
...
const seedSubjectContent = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({ message: 'Subject content seeded successfully.', content });
  } catch (error) {
...
const getSubjectContent = async (req, res) => {
  ...
  try {
    const content = await prisma.subjectContent.findMany({
      where: { ...(subject ? { subject } : {}), ...(form ? { form } : {}) },
      orderBy: { createdAt: 'desc' }
    });
